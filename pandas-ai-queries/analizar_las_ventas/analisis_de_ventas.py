...
def calculate_seasonal_variance_manually(df):
    ...
    seasonal_stats = (
        df.groupby(["product", "season"])["sales"].agg(["mean", "std"]).reset_index()
    )
    ...
    for product in df["product"].unique():
        ...
        cv = (
            np.std(seasonal_means) / np.mean(seasonal_means)
            if np.mean(seasonal_means) > 0
            else 0
        )
...
def visualize_results(df, variance_results):
    ...
    fig.suptitle(
        "Sales Data Seasonal Variance Analysis", fontsize=16, fontweight="bold"
    )
...
def main():
    ...
    for i, result in enumerate(variance_results, 1):
        print(
            f"{i}. {result['product']}: CV = {result['coefficient_of_variation']:.3f}"
        )
    ...
    visualize_results(df, variance_results)

    # Summary
    print("\n=== Summary ===")
    print(
        f"Analysis complete! The product with the highest seasonal variance is: {highest_variance_product}"
    )
    print(
        f"Coefficient of Variation: {variance_results[0]['coefficient_of_variation']:.3f}"
    )
    print(
        "\nThis indicates that this product's sales fluctuate the most between seasons,"
    )
    print(
        "making it important for inventory planning and seasonal marketing strategies."
    )
